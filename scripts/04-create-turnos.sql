-- Tabela de turnos (versão completa e corrigida)
...
-- Adicionar colunas se não existirem (para migração de tabelas existentes)
DO $$ 
BEGIN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'turnos_empresa_id_fkey' 
        AND table_name = 'turnos'
    ) THEN
        -- Try to add foreign key constraint (may fail if profiles table doesn't exist)
        BEGIN
            ALTER TABLE turnos ADD CONSTRAINT turnos_empresa_id_fkey 
            FOREIGN KEY (empresa_id) REFERENCES profiles(empresa_id) ON DELETE CASCADE;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not add foreign key constraint - profiles table may not exist yet';
        END;
    END IF;
END $$;
